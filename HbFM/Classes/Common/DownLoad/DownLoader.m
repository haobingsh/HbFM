...
#define kHeaderFilePath @"headerMsg.plist"
...
- (BOOL)checkLocalFile {
    ...
    self.fileCurrentSize = [FileTool getFileSizeWithPath:self.fileFullPath];
    
    if (self.fileCurrentSize > self.fileTotalSize) {
        // 删除文件, 并重新下载
        [FileTool removeFileAtPath:self.fileFullPath];
        return YES;
    }
...
+ (long long int)cacheFileSizeWithURL:(NSURL *)url {
    ...
    NSString *path = [kLocalPath stringByAppendingPathComponent:url.lastPathComponent];
    return  [FileTool getFileSizeWithPath:path];
...
+ (void)removeCacheFileWithURL:(NSURL *)url {
    ...
    NSString *path = [kLocalPath stringByAppendingPathComponent:url.lastPathComponent];
    [FileTool removeFileAtPath:path];
