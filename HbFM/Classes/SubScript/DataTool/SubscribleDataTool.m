...
@interface SubscribleDataTool()

@property (nonatomic, strong) FMSessionManager *sessionManager;

@end

@implementation SubscribleDataTool
singtonImplement(SubscribleDataTool)
...
- (void)getRecommandList:(void(^)(NSArray <NSString *>*recommendList, NSError *error))result {
    ...
    [self.sessionManager request:RequestTypeGet urlStr:url parameter:param resultBlock:^(id responseObject, NSError *error) {
        ...
        NSArray *menuePicMs = [Recommends mj_objectArrayWithKeyValuesArray:responseObject[@"data"][@"list"]];
...
- (void)getSubscribleList:(void(^)(NSArray <NSString *>*subscribleList, NSError *error))result{
    ...
    [self.sessionManager request:RequestTypeGet urlStr:url parameter:param resultBlock:^(id responseObject, NSError *error) {
        ...
        NSArray *menuePicMs = [Subscribe mj_objectArrayWithKeyValuesArray:plistData[@"data"][@"feedResult"][@"feedAlbumResults"]];
...
- (void)getAlbumMenuWithAlbumId:(NSString *)albumId result:(void(^)(AlbumMenu *albumMenu, NSError *error))result{
    ...
    [self.sessionManager request:RequestTypeGet urlStr:url parameter:param resultBlock:^(id responseObject, NSError *error) {
        ...
        [Tracks mj_setupObjectClassInArray:^NSDictionary *{
            return @{
                     @"list" : @"Track",
                     };
        }];
        AlbumMenu *albumMenu = [AlbumMenu mj_objectWithKeyValues:responseObject[@"data"]];
...
- (void)getAlbumDetailWithAlbumId:(NSString *)albumId result:(void(^)(AlbumDetail *albumDetail, NSError *error))result{
    ...
    [self.sessionManager request:RequestTypeGet urlStr:url parameter:param resultBlock:^(id responseObject, NSError *error) {
        [OtherRecommands mj_setupObjectClassInArray:^NSDictionary *{
            return @{
                     @"list" : @"OtherRecommand",
                     };
        }];
        AlbumDetail *albumDetail = [AlbumDetail mj_objectWithKeyValues:responseObject[@"data"]];
...
- (void)getAlbumTrackListWithAlbumId:(NSString *)albumId result:(void(^)(Tracks *tracks, NSError *error))result{
    ...
    [self.sessionManager request:RequestTypeGet urlStr:url parameter:param resultBlock:^(id responseObject, NSError *error) {
        ...
        Tracks *tracks = [Tracks mj_objectWithKeyValues:responseObject[@"data"]];
        result(tracks, error);
    }];
...
- (void)getDownLoadListAlbumId:(NSString *)albumId position:(NSInteger)position result:(void(^)(NSArray <NSString *>*downLoadList, NSError *error))result{
    ...
    [self.sessionManager request:RequestTypeGet urlStr:url parameter:param resultBlock:^(id responseObject, NSError *error) {
        ...
        NSArray *menuePicMs = [DownLoadModel mj_objectArrayWithKeyValuesArray:responseObject[@"tracks"][@"list"]];
