...
-(NSMutableArray<id<DanmuProtocol>> *)danmuMs
{
    if (_danmuMs == nil) {
...
- (void)checkDanmuSource {
    ...
    id<DanmuProtocol> danmuM = [self.danmuMs firstObject];
    ...
    if (danmuM.beginTime <= currentTime) {
        ...
        if (danmuM.liveTime <= [self.danDaos[danIndex] integerValue]) {
            ...
            // 重新按照startTime排序
            [self.danmuMs sortUsingComparator:^NSComparisonResult(id<DanmuProtocol> obj1, id<DanmuProtocol> obj2) {
