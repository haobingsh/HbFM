...
//#import "FriendListM.h"
//#import "DownLoadDataTool.h"
#import "PlayerVC.h"
#import "DanmuBackView.h"
#import "DanmuView.h"
...
@interface PlayerPane ()<DanmuBackViewRequiredProtocol>
{
    NSInteger _controlPaneCloseSec;
}


@property (strong, nonatomic) DanmuBackView *danmuBackView;
...
@property (nonatomic, strong) PlayListSheetView *playListView;

@end



@implementation PlayerPane

-(DanmuBackView *)danmuBackView
{
    if (_danmuBackView == nil) {
        _danmuBackView = [[DanmuBackView alloc] initWithFrame:self.middleImageView.bounds];
...
-(PlayListSheetView *)playListView
{
    if (_playListView == nil) {

        _playListView = [[NSBundle bundleForClass:[self class]] loadNibNamed:@"PlayListSheetView" owner:nil options:nil].firstObject;
...
-(void)awakeFromNib {
    ...
    NSString *path = [[NSBundle bundleForClass:[self class]] pathForResource:@"player_slider_playback_thumb@2x.png" ofType:nil inDirectory:@"FMPlayer.bundle"];

    [self.progressSlider setThumbImage:[UIImage imageWithContentsOfFile:path] forState:UIControlStateNormal];
    [self updateTimer];
}

-(void)setUserInfoM:(UserInfoModel *)userInfoM
{
...
- (void)update {
    ...
    float currentTime = [PlayerService shareInstance].currentPlayTime;
    float totalTime = [PlayerService shareInstance].currentTotalTime;
    ...
    if (page != ((int)currentTime / 10)) {
        ...
        [[PlayerDataTool shareInstance] getDanMuMsWithTrackId:self.trackM.trackId result:^(NSMutableArray<DanmuModel *> *danmuMs, BOOL isNewData) {
            // 代表加载的是新数据
            if (isNewData) {
...
-(void)setTrackM:(TrackInfoModel *)trackM
{
    ...
    self.playListView.clickItemBlock = ^(TrackInfoModel *trackM) {
        [PlayerVC shareInstanceWithAlbumId:trackM.albumId trackUid:trackM.trackId isUseCache:NO];
    };
...
//        [[XMGRadioService shareInstance] playCurrentAudio];
        ...
        [[PlayerService shareInstance] playWithURL:[NSURL URLWithString:filePath] withStateBlock:^(AudioState state) {
            switch (state) {
                case AudioPlaying:
...
- (IBAction)sliderUpInside:(UISlider *)slider {
    ...
    [[PlayerService shareInstance] seekProgress:slider.value];
...
- (IBAction)valueChange:(UISlider *)sender {
    ...
    float currentTime = sender.value * [[PlayerService shareInstance] currentTotalTime];
...
- (IBAction)preAudio {
    ...
    // 上一首
    TrackInfoModel *trackM = self.playListView.trackInfoMs[self.playListView.currentIndex --];
    [PlayerVC shareInstanceWithAlbumId:trackM.albumId trackUid:trackM.trackId isUseCache:NO];
}
...
- (IBAction)seekBack {
    ...
    [[PlayerService shareInstance] seekBack:15];
    _controlPaneCloseSec = 5;

}
- (IBAction)seekFast {

    [[PlayerService shareInstance] seekFast:15];
    _controlPaneCloseSec = 5;
...
-(NSTimeInterval)currentTime {
    ...
    return [PlayerService shareInstance].currentPlayTime;
...
- (UIView *)danmuViewWithDanmuM: (id<DanmuProtocol>)danmuM {
    ...
    DanmuView *danmuView = [DanmuView danmuViewWithDanmuM:danmuM];
...
- (void)danmuViewDidClick: (UIView *)danmuView pointInBackView: (CGPoint)point {
    ...
    if ([danmuView isKindOfClass:[DanmuView class]]) {
        DanmuView *danmuV = (DanmuView *)danmuView;
